...
{
    m_bodies.clear();
    b2Vec2 gravityaux = jsonToVec("gravity", worldValue);
    b2World* world = new b2World(b2Vec2(gravityaux.x,gravityaux.y*-1));
...
b2Body* b2dJson::j2b2Body(b2World* world, Json::Value bodyValue)
{
    ...
    b2Vec2 positionaux = jsonToVec("position", bodyValue);
    bodyDef.position = b2Vec2(positionaux.x,positionaux.y*-1);

    bodyDef.angle = jsonToFloat("angle", bodyValue );

    b2Vec2 linvelaux = jsonToVec("linearVelocity", bodyValue);
    bodyDef.linearVelocity = b2Vec2(linvelaux.x,linvelaux.y*-1);
    ...
    b2Vec2 centeraux = jsonToVec("massData-center", bodyValue);
    massData.center =  b2Vec2( centeraux.x,centeraux.y*-1);
...
b2Fixture* b2dJson::j2b2Fixture(b2Body* body, Json::Value fixtureValue)
{
    ...
    if ( !fixtureValue["circle"].isNull() ) {
        ...
        //b2Vec2 circlecenteraux = jsonToVec("center", fixtureValue["circle"]);
        //cout<<circlecenteraux.x<<" - "<<circlecenteraux.y<<endl;
        ...
        edgeShape.m_vertex1 = b2Vec2(vertex1aux.x,vertex1aux.y*-1);

        b2Vec2 vertex2aux = jsonToVec("vertex2", fixtureValue["edge"]);
        edgeShape.m_vertex2 = b2Vec2(vertex2aux.x,vertex2aux.y*-1);
        ...
        if ( numVertices > b2_maxPolygonVertices ) {
            ...
            b2Vec2 vertex1aux = jsonToVec("vertices", fixtureValue["polygon"], 0);
            edgeShape.m_vertex1 = b2Vec2(vertex1aux.x,vertex1aux.y*-1);

            b2Vec2 vertex2aux = jsonToVec("vertices", fixtureValue["polygon"], 1);
            edgeShape.m_vertex2 = b2Vec2(vertex2aux.x,vertex2aux.y*-1);
            fixtureDef.shape = &edgeShape;
            fixture = body->CreateFixture(&fixtureDef);
        }
